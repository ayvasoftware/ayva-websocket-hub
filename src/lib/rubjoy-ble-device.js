/**
 * Small convenience class for easily connecting to a Rubjoy Bluetooth LE device from an Electron app
...
export default class RubjoyBLEDevice {
  ...
  _customDisconnectListener = null;
  ...
  async requestConnection () {
    const device = await this._bluetooth.requestDevice({
      ...
      // }],
      // We need to accept all devices in Electron since filtering doesn't work and selection is automatically done by the backend.
      acceptAllDevices: true,
      optionalServices: [RubjoyBLEDevice.SERVICE_UUID],
    });
    ...
    const disconnectListener = () => {
      ...
      if (this._customDisconnectListener) {
        this._customDisconnectListener();
      }
    };
  ...
  onDisconnect (listener) {
    this._customDisconnectListener = listener;
  }
